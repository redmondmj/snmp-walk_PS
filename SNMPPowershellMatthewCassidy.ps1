...
$oidPage = "1.3.6.1.4.1.1347.43.10.1.1.12.1"
#$TonerPercent = tonerLevel($oidToner)
function tonerLevel {
    param (
        $oidToner
    )
...
do { 
    ...
    "6" {Write-Output "The currennt toner level for the printer is $TonerPercent%."| Out-File -FilePath ".\PrinterData-$IP.txt" -Append}
    "7" {invoke-snmpwalk -IP $IP -OID $oidLocation | Out-File -FilePath ".\PrinterData-$IP.txt" -Append} 
    "8" {Invoke-snmpwalk -IP $IP -OID $oidPage| Out-File -FilePath ".\PrinterData-$IP.txt" -Append}
