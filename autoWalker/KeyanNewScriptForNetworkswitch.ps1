...
do { #Input validation for IP
    ...
    write-host ""
    #get input from user
    $IP = read-host "Enter IP for the community EntMan"
...
do { 
    ...
    do {
        ...
        write-host "2 - Switch location"
        write-host "3 - Switch description"
    ...
    "1" {invoke-snmpwalk -IP $IP -Community EntMan -OID $oidOwner | Out-File -FilePath ".\SwitchData-$IP.txt" -Append}
    "2" {invoke-snmpwalk -IP $IP -Community EntMan -OID $oidLocation | Out-File -FilePath ".\SwitchData-$IP.txt" -Append}
    "3" {invoke-snmpwalk -IP $IP -Community EntMan -OID $oidDescription | Out-File -FilePath ".\SwitchData-$IP.txt" -Append}
    "4" {invoke-snmpwalk -IP $IP -Community EntMan -OID $oidUptime | Out-File -FilePath ".\SwitchData-$IP.txt" -Append}
